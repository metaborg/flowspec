...
import mb.flowspec.java.interpreter.TransferFunction;
import mb.flowspec.java.interpreter.UnreachableException;
...
public class FixedPoint {
    ...
    private void solveFlowSensitiveProperty(ICompleteControlFlowGraph<CFGNode> cfg,
        ...
        for (CFGNode n : cfg.nodes()) {
            setProperty(n, prop, (mb.flowspec.java.interpreter.values.Set<IStringTerm>) metadata.lattice().bottom());
        }
        ...
        switch (metadata.dir()) {
            case Forward: {
                for (CFGNode n : cfg.startNodes()) {
                    setProperty(n, prop, new mb.flowspec.java.interpreter.values.Set<>());
